...
# The auxiliary data lives in a private GitLab repository at git.km3net.de which is
# only reachable from GitLab CI (via CI_JOB_TOKEN) or locally (via SSH). On the GitHub
# Actions mirror neither is available, so these tests are skipped there.
if haskey(ENV, "GITHUB_ACTIONS")
    @info "Skipping KM3Aux extension tests: no access to git.km3net.de on GitHub Actions"
else
